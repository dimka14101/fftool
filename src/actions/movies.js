...
import {
    ...
    MOVIES_REQ, MOVIES_RES, MOVIES_ERR, //main page movies
    RECOM_REQ, RECOM_RES, RECOM_ERR, //recomendations for movie
    MOVIE_REQ, MOVIE_RES, MOVIE_ERR, //one movie
    BY_GENRES_REQ, BY_GENRES_RES, BY_GENRES_ERR, BY_GENRES_CLEAR, //movies by genre
    TOP_MOVIES_REQ, TOP_MOVIES_RES, TOP_MOVIES_ERR, //top movies
} from '../Constants';

export const getMovies = (pageId) => (dispatch) => {
    dispatch({ type: MOVIES_REQ });
    axios.get('/movie/top_rated?page='+pageId)
        .then(res => {
            console.log(res);
...
export const filterByGenre = (genres,pageId) => (dispatch) => {
    ...
    axios.get('/discover/movie?sort_by=popularity.desc&with_genres=' + genres+'&page='+pageId)
        .then(res => {
            console.log(res);
